...
- [ ] Task: Create `composables/useDuration.ts` to encapsulate the date parsing and diffing logic.
- [ ] Task: Implement `formatDuration` helper to output `Xd Xh`.
- [ ] Task: Write unit tests for `useDuration` covering various date formats (ISO string, Excel serial) and status scenarios (Open vs Closed).
- [ ] Task: Conductor - User Manual Verification 'Logic & Utility' (Protocol in workflow.md)
